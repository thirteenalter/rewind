...
<div class="min-h-screen bg-[#0a0a0a] font-['Montserrat'] text-white pb-20">
  {#if featured}
    <section class="relative w-full h-[60vh] overflow-hidden">
      ...
      >
        <div
          class="inline-flex items-center backdrop-blur-md bg-red-600/20 border border-red-500/30 rounded-full px-4 py-1.5 text-[10px] font-bold uppercase tracking-[0.5em] mb-4 w-fit text-red-500"
        >
          Hot Trending
        </div>
        ...
        >
          <span class="text-white font-bold">{total} Trending Items</span>
          <span class="w-1 h-1 bg-white/20 rounded-full"></span>
          <span>Top Picks by Users</span>
        </p>
      </div>
    </section>
  {/if}
